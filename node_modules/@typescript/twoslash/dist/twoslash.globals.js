    ...
    if (match = valuedConfigRegexp.exec(codeLines[i])) {
      if (customTags.includes(match[1])) {
        tags.push({
          name: match[1],
          line: i,
          annotation: codeLines[i].split("@" + match[1] + ": ")[1]
        });
        codeLines.splice(i, 1);
      }
    }
...
function twoslasher(code, extension, options) {
    ...
    if (!handbookOptions.showEmit) {
      ...
      for (var _iterator3 = _createForOfIteratorHelperLoose(identifiers), _step3; !(_step3 = _iterator3()).done;) {
        ...
        if (quickInfo && quickInfo.displayParts) {
          ...
          var position = span.start + fileContentStartIndexInModifiedFile;
  ...
  if (handbookOptions.showEmit) {
    ...
    var emitSourceFilename = fsRoot + emitFilename.replace(".jsx", "").replace(".js", "").replace(/\.d\.([^\.]+\.)?[cm]?ts$/i, "").replace(".map", "");
